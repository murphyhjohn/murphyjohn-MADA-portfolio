...
# pivot longer for table
dat6 <- dat5 %>%
  select(
    -total_emissions
  ) %>%
  tidyr::pivot_longer(
    cols = c(-food_items, -food_group),
    names_to = "category",
    values_to = "emissions"
  )

# average over groups and sum the averages
dat7 <- dat6 %>%
  group_by(category, food_group) %>%
  summarise(
    avg_emission = round(mean(emissions), 2),
    .groups = 'drop'
  ) %>%
  group_by(food_group) %>%
  mutate(
    tot_mean_emission = sum(avg_emission),
    emission_percentage = round((avg_emission / tot_mean_emission) * 100, 2)
  ) %>% 
  ungroup() %>%
  tidyr::pivot_wider(
    names_from = category,
    values_from = c(avg_emission, emission_percentage),
    names_glue = "{.value}_{category}"
  ) %>%
  mutate(
    # Combine avg_emission and emission_percentage in the desired format
    across(
      starts_with("avg_emission"), 
      ~paste0(.x, " (", 
              round(.x / sum(.x) * 100, 2), "%)"), 
      .names = "{.col}"
    )
